...
def create_mars_cluster(odps, worker_num=1, worker_cpu=8, worker_mem=32, cache_mem=None, disk_num=1,
                        ...
                        notebook=False, instance_id=None, **kw):
    """
...
def to_mars_dataframe(odps, table_name, shape=None, partition=None, chunk_bytes=None,
    ...
    odps_params = dict(
        project=odps.project, endpoint=cupid_internal_endpoint or options.cupid.runtime.endpoint)
    ...
    return read_odps_table(odps.get_table(table_name), shape, partition=partition,
                           chunk_bytes=chunk_bytes, sparse=sparse,
...
def persist_mars_dataframe(odps, df, table_name, overwrite=False, partition=None, write_batch_size=None,
    ...
    odps_params = dict(project=odps.project,
                       endpoint=cupid_internal_endpoint or options.cupid.runtime.endpoint)
...
def run_mars_job(odps, func, args=(), kwargs=None, retry_when_fail=False, n_output=None, **kw):
    ...
    try:
        ...
        r.op.extra_params['endpoint'] = cupid_internal_endpoint or options.cupid.runtime.endpoint
        r.execute()
    finally:
        client.stop_server()
    ...
    def wrapper(ctx, op):
        ...
        try:
            if context() is None:
                ...
                env['ODPS_ENDPOINT'] = str(op.extra_params.get('endpoint', None) or env['ODPS_RUNTIME_ENDPOINT'])
